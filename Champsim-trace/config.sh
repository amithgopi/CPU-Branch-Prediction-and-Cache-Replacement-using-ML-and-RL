...
with open('inc/cache_modules.inc', 'wt') as wfp:
    ...
    wfp.write('\n'.join('uint32_t {1}(uint32_t, uint64_t, uint32_t, const BLOCK*, uint64_t, uint64_t, uint32_t, string);'.format(*r) for r in repl_victims))
    wfp.write('\nuint32_t impl_replacement_find_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK* current_set, uint64_t ip, uint64_t full_addr, uint32_t type, string origin)\n{\n    ')
    wfp.write('\n    '.join('if (repl_type == repl_t::{}) return {}(cpu, instr_id, set, current_set, ip, full_addr, type, origin);'.format(*r) for r in repl_victims))
    wfp.write('\n    throw std::invalid_argument("Replacement policy module not found");')
    wfp.write('\n}\n')
    wfp.write('\n')

    wfp.write('\n'.join('void {1}(uint32_t, uint32_t, uint32_t, uint64_t, uint64_t, uint64_t, uint32_t, uint8_t, string);'.format(*r) for r in repl_updates))
    wfp.write('\nvoid impl_replacement_update_state(uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr, uint64_t ip, uint64_t victim_addr, uint32_t type, uint8_t hit, string origin)\n{\n    ')
    wfp.write('\n    '.join('if (repl_type == repl_t::{}) return {}(cpu, set, way, full_addr, ip, victim_addr, type, hit, origin);'.format(*r) for r in repl_updates))
    wfp.write('\n    throw std::invalid_argument("Replacement policy module not found");')
    wfp.write('\n}\n')
    wfp.write('\n')
