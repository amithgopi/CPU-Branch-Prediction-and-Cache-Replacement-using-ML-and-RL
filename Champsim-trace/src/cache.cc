...
void CACHE::handle_writeback()
{
  while (writes_available_this_cycle > 0) {
    ...
    if (way < NUM_WAY) // HIT
    {
      impl_replacement_update_state(handle_pkt.cpu, set, way, fill_block.address, handle_pkt.ip, 0, handle_pkt.type, 1, "handle_writeback");
      ...
      if (handle_pkt.type == RFO && handle_pkt.to_return.empty()) {
        ...
        if (way == NUM_WAY)
          way = impl_replacement_find_victim(handle_pkt.cpu, handle_pkt.instr_id, set, &block.data()[set * NUM_WAY], handle_pkt.ip, handle_pkt.address,
                                             handle_pkt.type, "handle_writeback");
...
void CACHE::readlike_hit(std::size_t set, std::size_t way, PACKET& handle_pkt)
{
  ...
  impl_replacement_update_state(handle_pkt.cpu, set, way, hit_block.address, handle_pkt.ip, 0, handle_pkt.type, 1, "readlike_hit");
...
bool CACHE::filllike_miss(std::size_t set, std::size_t way, PACKET& handle_pkt)
{
  ...
  // update replacement policy
  impl_replacement_update_state(handle_pkt.cpu, set, way, handle_pkt.address, handle_pkt.ip, 0, handle_pkt.type, 0, "filllike_miss");
